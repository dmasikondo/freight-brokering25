        ...
        <div x-data="{
            ...
            <div class="flex flex-col md:flex-row gap-8">
                ...
                <div class="md:w-1/3 text-center mb-8 md:mb-0 space-y-4">
                        ...
                        @foreach ($user->roles as $role)
                            ...
                            @if ($role->pivot?->classification)
                                @php
                                    ...
                                    $classificationText =
                                        $role->pivot->classification === 'real_owner' ? 'Real Owner' : 'Broker / Agent';
                                    $iconColor =
                                        $role->pivot->classification === 'real_owner'
                                            ? 'text-yellow-400'
                                            : 'text-blue-400';
                                @endphp
                    ...
                    @can('update', $user)
                        <button
                            class="w-full mt-4 bg-indigo-800 text-white px-4 py-2 rounded-lg hover:bg-blue-900 transition-colors duration-300">Edit
                            Profile
                        </button>
                        <p x-show="hasCreator" x-text="'Created By: ' + hasCreator"
                            class="text-gray-400 text-xs font-extralight"></p>
                    @endcan
                ...
                <div class="md:w-2/3 md:pl-8">
                    ...
                    <div x-show="activeTab === 'activity'" class="space-y-6">
                        ...
                        <template x-if="isShipper || isMarketing || isOperationsLA">
                            <div>
                                <h2 class="text-xl font-semibold text-indigo-800 dark:text-white mb-4">Shipping Activity
                                </h2>
                                <div class="flex flex-wrap gap-2">
                                    <span
                                        class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm">Sent</span>
                                    <span
                                        class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm">In
                                        Transit</span>
                                    <span
                                        class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm">Delivered</span>
                                    <span
                                        class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm">Awaiting
                                        Approval</span>
                                    <span
                                        class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm">Draft</span>
                                    <template x-if="isMarketing || isOperationsLA">
                                        <span
                                            class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm"
                                            title="Total No. of Shippers">
                                            Registered ( {{ $user->createdUsers->count() }} )
                                        </span>
                                        <span
                                            class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm"
                                            title="Total No. of Invoices">Invoices</span>
                                        <span
                                            class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm"
                                            title="Pending Invoices">Pending</span>
                                        <span
                                            class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm"
                                            title="Overdue Invoices">Overdue</span>
                                        <span
                                            class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm"
                                            title="Paid Invoices">Paid</span>
                                    </template>
                                </div>
                            </div>
                        </template>
                        ...
                        <template x-if="isCarrier || isProcurementLA || isOperationsLA">
                            <div>
                                <h2 class="text-xl font-semibold text-indigo-800 dark:text-white mb-4">Carrier Activity
                                </h2>
                                <div class="flex flex-wrap gap-2">
                                    <template x-if="isCarrier">
                                        <span
                                            class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm">Directors
                                            Info</span>
                                        <span
                                            class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm">Trade
                                            References</span>
                                        <span
                                            class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm">Fleet
                                            Info</span>
                                        <span
                                            class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm">Bids</span>
                                    </template>
                                    <template x-if="isProcurementLA || isOperationsLA">
                                        <span
                                            class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm"
                                            title="Total No. of Carriers">Registered
                                            ( {{ $user->createdUsers->count() }} )
                                        </span>
                                        <span
                                            class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm"
                                            title="Incomplete Registrations">Incomplete</span>
                                        <span
                                            class="{{ implode(' ', $getRandomColor()) }} px-3 py-1 rounded-full text-sm"
                                            title="No. of Published Vehicles">Vehicles</span>
                                    </template>
                                </div>
                            </div>
                        </template>
                            ...
                            <ul class="space-y-4 text-gray-700 dark:text-gray-300">
                                <li class="flex items-center gap-3">
                                    ...
                                    <div class="flex flex-col">
                                        <span
                                            class="font-semibold text-sm text-gray-500 dark:text-gray-400">Email</span>
                                ...
                                <li class="flex items-center gap-3">
                                    ...
                                    <div class="flex flex-col">
                                        <span
                                            class="font-semibold text-sm text-gray-500 dark:text-gray-400">Phone</span>
                                        <span class="font-medium">{{ $user->phone_type }}:
                                            {{ $user->contact_phone }}</span>
                                    </div>
                                </li>
                                <template x-if="hasWhatsapp">
                                    <li class="flex items-center gap-3">
                                        ...
                                        <div class="flex flex-col">
                                            <span
                                                class="font-semibold text-sm text-gray-500 dark:text-gray-400">Whatsapp</span>
                                ...
                                <template x-if="hasLocation">
                                    <li class="flex items-center gap-3">
                                        <x-graphic name="location-marker"
                                            class="size-5 text-green-400 dark:text-green-800" />
                                        <div class="flex flex-col">
                                            <span
                                                class="font-semibold text-sm text-gray-500 dark:text-gray-400">Address</span>
