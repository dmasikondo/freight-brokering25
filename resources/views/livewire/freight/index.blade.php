    ...
    @else
        <div class="flex flex-col lg:flex-row lg:flex-wrap lg:gap-4">
            @foreach ($this->getFreights as $freight)
                <div
                    class="w-full lg:w-5/12 mx-2 border-b pb-4 mb-4 border border-gray-300 rounded-lg shadow-md last:border-0 relative p-8">
                    <x-card.feature-card :feature="[
                        'color' => $freight->status->color(),
                        'title' => implode(', ', $freight->goods->pluck('name')->toArray()),
                        'icon' => 'cube',
                        ...
                        @auth
                            <div class="mr-2 absolute top-0 right-0">
                                <flux:dropdown position="right" align="end">
                                    ...
                                    <flux:navmenu>
                                        <flux:navmenu.item href="{{ route('freights.edit', ['freight' => $freight->id]) }}"
                                            icon="pencil-square">Edit Freight</flux:navmenu.item>
                        ...
                        <div class="">
                            <flux:badge size='sm' :color="$freight->status->color()">
                            ...
                            <div class="my-6 space-y-2 space-x-2">
                                <p class='flex space-x-2'> <flux:icon.clipboard-document-list />
                                    {{ $freight->description }}
                                </p>
                                    ...
                                    <div class="my-6 flex space-x-2">
                                        ...
                                        <div>
                                            ...
                                            <flux:text size='sm' variant='subtle'>
                                                {{ $freight->updated_at->diffForHumans() }}</flux:text>
                                        </div>
