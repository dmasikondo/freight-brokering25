...
new class extends Component {
    ...
    public function updatingWorksheetType(): void
    {
        $this->resetPage();
    }
    ...
    public function with(): array
    {
        ...
        $query = WorksheetHeader::query()
            ...
            ->withCount(['entries', 'entries as completed_entries_count' => fn($q) => $q->whereNotNull('completed_at')]);
        ...
        if ($this->author_search) {
            $query->whereHas('user', fn($q) => $q->whereAny(['contact_person', 'email', 'organisation'], 'like', '%' . $this->author_search . '%'));
        }
        ...
        match ($this->reminder_filter) {
            'overdue' => $query->whereHas('entries', fn($q) => $q->whereNotNull('reminder_at')->where('reminder_at', '<', now())),
            'forthcoming' => $query->whereHas('entries', fn($q) => $q->whereNotNull('reminder_at')->where('reminder_at', '>', now())),
            default => null,
        };
        ...
        return [
            ...
            'available_staff' => User::where('id', '!=', auth()->id())
                ->whereHas('roles', fn($q) => $q->whereIn('name', ['superadmin', 'admin', 'marketing logistics associate', 'procurement logistics associate', 'operations logistics associate', 'logistics operations executive']))
                ->when($this->staff_search, fn($q) => $q->whereAny(['users.contact_person', 'users.email'], 'like', "%{$this->staff_search}%"))
                ->get(),
        ];
    }
...
<div class="min-h-screen bg-lime-50/50 dark:bg-slate-950">
    ...
    <div class="sticky top-0 z-20 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
        <div class="max-w-7xl mx-auto px-4 md:px-8 h-14 flex items-center justify-between gap-4">
            <div>
                ...
                <p class="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">Partner interactions & logistics leads
                </p>
            </div>
            <flux:button href="{{ route('worksheets.create') }}" wire:navigate icon="plus" color="lime"
                variant="primary">
                New Worksheet
            </flux:button>
        </div>
        ...
        <div class="max-w-7xl mx-auto px-4 md:px-8 flex items-center gap-0 overflow-x-auto">
            ...
            @endphp
            @foreach ($tabs as $tab)
                <button type="button" wire:click="setType('{{ $tab['value'] }}')" wire:loading.attr="disabled"
                    wire:target="setType"
    ...
    <div class="max-w-7xl mx-auto px-4 md:px-8 py-6 space-y-5">
        ...
        ════════════════════════════════════════════════════════ --}}
        <details
            class="group bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <summary
                class="cursor-pointer flex items-center justify-between px-5 py-4 select-none
            ...
            <div class="px-5 pb-5 space-y-4 border-t border-slate-100 dark:border-slate-800 pt-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 items-end">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <div class="flex items-center gap-2">
                            <span
                                class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Show</span>
        ...
        <div class="space-y-3" wire:loading.class="opacity-60">
            @forelse ($worksheets as $ws)
                ...
                <div
                    class="relative bg-white dark:bg-slate-900 rounded-2xl border shadow-sm
                    ...
                    @if ($isCollaborator)
                        <div
                            class="absolute -top-3 left-5 flex items-center gap-1.5 px-3 py-1
                    ...
                    <div class="p-5 md:p-6">
                        <div class="flex flex-col md:flex-row justify-between items-start gap-4">
                            ...
                            <div class="flex-grow min-w-0">
                                <div class="flex items-center gap-2 flex-wrap">
                                    ...
                                    <span
                                        class="px-1.5 py-0.5 text-[9px] font-black uppercase rounded
                                        {{ $ws->is_completed
                                ...
                                {{-- Authored by --}}
                                <p
                                    class="text-[10px] text-slate-400 dark:text-slate-500 mt-1.5 flex items-center gap-1">
                                    <flux:icon.user variant="micro" class="text-slate-300 dark:text-slate-600" />
                                    <span
                                        class="{{ $isOwner ? 'text-emerald-600 dark:text-emerald-400 font-bold' : '' }}">
                                ...
                                @if (!$ws->is_completed)
                                    ...
                                    <p
                                        class="text-[9px] font-black uppercase tracking-widest mt-1 flex items-center gap-1
                                        {{ $seqMins === 0 ? 'text-red-400 dark:text-red-500' : 'text-amber-500 dark:text-amber-400' }}">
                                ...
                                @if ($otherCollabs->isNotEmpty())
                                    <div class="mt-3 flex flex-wrap items-center gap-1.5">
                                        <span
                                            class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase">Team:</span>
                                        @foreach ($otherCollabs as $collab)
                                            <a href="{{ route('users.show', $collab->slug) }}" wire:navigate
                                                ...
                                                <div
                                                    class="h-4 w-4 rounded-full bg-emerald-100 dark:bg-emerald-900 flex items-center
                                                ...
                                                <span
                                                    class="text-[9px] font-bold text-slate-600 dark:text-slate-400
                            ...
                            <div class="flex items-center gap-2 flex-shrink-0 w-full md:w-auto justify-end">
                                @if ($isOwner)
                                    <button type="button" wire:click="openShareModal({{ $ws->id }})"
                        ...
                        {{-- Progress bar --}}
                        <x-worksheet._progress-bar :completed="$ws->completed_entries_count" :total="$ws->entries_count" :muted="$ws->is_completed"
                            class="mt-4" />
                    </div>
                </div>
            ...
            @empty
                <div
                    class="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900
                    ...
                    <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Adjust your filters or start a new
                        worksheet.</p>
                    <flux:button href="{{ route('worksheets.create') }}" wire:navigate icon="plus" color="lime"
                        variant="primary">
                        New Worksheet
                ...
                @foreach ($available_staff as $staff)
                    <div
                        ...
                        <div class="flex items-center gap-3">
                            <div
                                class="h-9 w-9 rounded-full bg-emerald-100 dark:bg-emerald-900 flex items-center
                            ...
                            <div>
                                ...
                                <span
                                    class="text-[9px] text-emerald-600 dark:text-emerald-400 font-black uppercase
                        ...
                        <flux:checkbox wire:key="staff-{{ $staff->id }}-{{ count($selectedStaff) }}"
                            ...
                            wire:click="toggleShare({{ $staff->id }})" class="cursor-pointer" />
                    </div>
                @endforeach
            </div>
