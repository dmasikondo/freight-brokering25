...
new class extends Component {
    ...
    private function getUserStats()
    {
        ...
        // Conditionally add the 'superadmin' key *after* defining the array
        if ($this->user->hasRole('superadmin')) {
            $backendStaff['superadmin'] = User::whereHas('roles', function ($q) {
                $q->where('name', 'superadmin');
            })->count();
        }
    ...
    {
        return Freight::where('status', 'published')->where('shipment_status', '!=', 'delivered')->count();
    }

    private function getAvailableLanesCount()
    {
        return Lane::where('status', 'published')->where('vehicle_status','inapplicable')->count();
    }
                    ...
                    <div class="space-y-4">
                        <flux:link href="{{ route('users.index', ['search' => 'carrier']) }}" wire:navigate>
                            <div class="flex justify-between items-center p-3 bg-blue-50 rounded-lg mb-2">
                                ...
                                <span
                                    class="text-lg font-bold text-blue-600">{{ $stats['users']['carriers'] ?? 0 }}</span>
                            </div>
                        </flux:link>
                        <flux:link href="{{ route('users.index', ['search' => 'shipper']) }}" wire:navigate>
                            <div class="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                                ...
                                <span
                                    class="text-lg font-bold text-green-600">{{ $stats['users']['shippers'] ?? 0 }}</span>
                            </div>
                        </flux:link>
                        ...
                        <div class="mt-4">
                            ...
                            <div class="grid grid-cols-1 gap-2">
                                @foreach ($stats['users']['backend_staff'] ?? [] as $role => $count)
                                 <flux:link href="{{ route('users.index', ['search' => str_replace('_', ' ', $role)]) }}" wire:navigate>
                                        <div class="flex justify-between items-center py-2 px-3 bg-gray-50 rounded">
                                            <span
                                                class="text-sm text-gray-600 capitalize">{{ str_replace('_', ' ', $role) }}</span>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <a href="#" class="text-blue-600 hover:text-blue-800 text-sm font-medium">View
                                All</a>
                        </div>
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-2xl font-bold text-gray-900">
                                    {{ $stats['active_contracts']['loads_in_progress'] ?? 0 }}</p>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <div class="flex space-x-2">
                                <a href="{{ route('territories.create') }}" wire:navigate
                                    class="inline-flex items-center px-3 py-1 bg-blue-600 text-white text-sm rounded-md hover:bg-blue-700">
                                    Add Territory
                                </a>
                                <a href="{{ route('territories.index') }}" wire:navigate
                                    class="inline-flex items-center px-3 py-1 border border-gray-300 text-gray-700 text-sm rounded-md hover:bg-gray-50">
                                    View All
                                </a>
                            </div>
                        </div>
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                <p class="text-2xl font-bold text-gray-900">{{ $stats['territories']['total'] ?? 0 }}
                                </p>
                            ...
                            <div class="space-y-2">
                                ...
                                @foreach ($stats['territories']['recent'] as $territory)
                                    <div class="flex justify-between items-center py-2 px-3 bg-gray-50 rounded">
                                        ...
                                        <span
                                            class="text-xs text-gray-500">{{ $territory->created_at->diffForHumans() }}</span>
                                    </div>
                                @endforeach
                            </div>
                        @endif
